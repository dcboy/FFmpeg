...
static int hl_aac_set_extradata(AVCodecContext *avctx, AMediaFormat *format)
{
  ...
  uint16_t val = (0xFFFF & (AOT_AAC_LC << 11)) |
                 ...
                 (0xFFFF & (avctx->ch_layout.nb_channels << 3));
  ...
  hi_logi(avctx, "%s samplerate = %d, channels = %d, csd-0 = %02X%02X", __FUNCTION__, avctx->sample_rate, avctx->ch_layout.nb_channels, esds[0], esds[1]);
  return 0;
}
...
static int hlmediacodec_decode_fill_format(AVCodecContext *avctx, AMediaFormat *mediaformat)
{
  ...
  do
  {
    ...
    {
      uint32_t frame_max_size = av_samples_get_buffer_size(NULL, avctx->ch_layout.nb_channels, avctx->sample_rate, avctx->sample_fmt, 1);
      ...
      AMediaFormat_setInt32(mediaformat, AMEDIAFORMAT_KEY_CHANNEL_COUNT, avctx->ch_layout.nb_channels);
    }
    ...
    hi_logi(avctx, "%s %d mime: %s timeout: (in: %d ou: %d) times: (in: %d ou: %d) "
                   "width: %d height: %d pix_fmt: %d sample_fmt: %d",
...
int hlmediacodec_fill_context(AMediaFormat *mediaformat, AVCodecContext *avctx)
{
  if (av_codec_is_decoder(avctx->codec))
  {
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
      ...
      if (media_channel)
      {
        avctx->ch_layout.nb_channels = media_channel;
      }
    }
    else if (avctx->codec_type == AVMEDIA_TYPE_VIDEO)
    {
...
int hlmediacodec_decode_buffer_to_frame(AVCodecContext *avctx, AMediaCodecBufferInfo bufferinfo, AVFrame *frame)
{
  ...
  do
  {
    if (avctx->codec_type == AVMEDIA_TYPE_AUDIO)
    {
      uint32_t frame_data_size = av_samples_get_buffer_size(NULL, frame->ch_layout.nb_channels, frame->nb_samples, (enum AVSampleFormat)frame->format, 1);
      ...
      if ((ret = avcodec_fill_audio_frame(frame, frame->ch_layout.nb_channels, (enum AVSampleFormat)frame->format, ctx->buffer, frame_data_size, 1)) < 0)
      {
        hi_loge(avctx, "%s %d avcodec_fill_audio_frame fail (%d) format: %d channels: %d format: %d", __FUNCTION__, __LINE__, ret, frame->format, frame->ch_layout.nb_channels, frame->format);
        break;
      }
    }
    else if (avctx->codec_type == AVMEDIA_TYPE_VIDEO)
...
int hlmediacodec_get_buffer_size(AVCodecContext *avctx)
{
  if (avctx->codec_type == AVMEDIA_TYPE_AUDIO)
  {
    return av_samples_get_buffer_size(NULL, avctx->ch_layout.nb_channels, avctx->frame_size, avctx->sample_fmt, 1);
  }
  else if (avctx->codec_type == AVMEDIA_TYPE_VIDEO)
  {
