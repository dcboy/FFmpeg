...
static av_cold int hlmediacodec_encode_init(AVCodecContext *avctx)
{
  ...
  do
  {
    hi_logi(avctx, "%s %d init start globalHdr: [%d %s] rc_mode: %d",
            __FUNCTION__, __LINE__, avctx->flags,
            (avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER) ? "yes" : "no",
            ctx->rc_mode);
    ...
    {
      hi_loge(avctx, "%s %d mediacodec_encode_fill_format failed (%d)!",
    ...
    {
      hi_loge(avctx, "%s %d mediacodec_encode_header failed (%d)!",
              __FUNCTION__, __LINE__, ret);
      break;
    }
    ...
    {
      ret = AVERROR_EXTERNAL;
      hi_loge(avctx, "%s %d codec (%d) unsupport!", __FUNCTION__, __LINE__,
              avctx->codec_id);
      break;
    }
    ...
    {
      ret = AVERROR_EXTERNAL;
      hi_loge(avctx, "%s %d AMediaCodec_createEncoderByType failed!",
              __FUNCTION__, __LINE__);
      break;
    }

    hi_logi(avctx, "%s %d AMediaCodec_configure %s format %s", __FUNCTION__,
            __LINE__, mime, AMediaFormat_toString(ctx->mediaformat));

    media_status_t status = AMEDIA_OK;
    if ((status =
             AMediaCodec_configure(ctx->mediacodec, ctx->mediaformat, NULL, 0,
                                   HLMEDIACODEC_CONFIGURE_FLAG_ENCODE)))
    {
      ret = AVERROR(EINVAL);
      hi_loge(avctx, "%s %d AMediaCodec_configure failed (%d) !", __FUNCTION__,
              __LINE__, status);
      break;
    }
    ...
    {
      ret = AVERROR(EIO);
      hi_loge(avctx, "%s %d AMediaCodec_start failed (%d) !", __FUNCTION__,
              __LINE__, status);
      break;
    }
...
static int hlmediacodec_enc_send(AVCodecContext *avctx)
{
  ...
  do
  {
    ...
    hi_logd(avctx, "ffmpeg encode.c %s %d %d %d", __FUNCTION__, __LINE__,
            get_ret, ctx->frame->pkt_size);
    if (get_ret != 0)
    {
      ...
      if (get_ret == AVERROR_EOF)
      {
        ...
        hi_logd(avctx, "ffmpeg %s %d ff_encode_get_frame eof", __FUNCTION__,
                __LINE__);
      }
      else
      {
        ret = AVERROR(EAGAIN);
        hi_logd(avctx, "ffmpeg %s %d ff_encode_get_frame fail (%d)",
                __FUNCTION__, __LINE__, get_ret);
        break;
      }
    }
    else
    ...
    while (true)
    {
      ...
      if (!ctx->in_eof)
      {
        ...
        int copy_ret = av_image_copy_to_buffer(
            in_buffer, in_buffersize, (const uint8_t **)ctx->frame->data,
            ctx->frame->linesize, ctx->frame->format, ctx->frame->width,
            ctx->frame->height, 1);
        ...
        int64_t pts =
            av_rescale_q(ctx->frame->pts, avctx->time_base, AV_TIME_BASE_Q);
        int64_t duration = av_rescale_q(ctx->frame->pkt_duration,
                                        avctx->time_base, AV_TIME_BASE_Q);
        in_bufidx = AMediaCodec_queueInputBuffer(ctx->mediacodec, in_bufidx, 0,
                                                 copy_size, pts, 0);
        ctx->in_pts = pts;
      ...
      {
        in_bufidx = AMediaCodec_queueInputBuffer(
            ctx->mediacodec, in_bufidx, 0, 0, 0,
            HLMEDIACODEC_BUFFER_FLAG_END_OF_STREAM);
        ctx->in_pts += ctx->in_duration;
        hi_logi(avctx, "%s %d AMediaCodec_queueInputBuffer eof flush",
                __FUNCTION__, __LINE__);
      }
  ...
  hi_logd(avctx, "%s %d h264_hlmediacodec (%d) ctx->frame address %p", __FUNCTION__, __LINE__, ret, ctx->frame);
  return ret;
}
// return frames
...
static int hlmediacodec_enc_recv(AVCodecContext *avctx, AVPacket *pkt)
{
  ...
  hi_logd(avctx, "%s %d hlmediacodec_enc_recv encoded copy data ou_times (%d)", __FUNCTION__, __LINE__, ou_times);
  ...
  while (true)
  {
    ...
    if (ou_bufidx >= 0)
    {
      ...
      int64_t pts = av_rescale_q(bufferInfo.presentationTimeUs, AV_TIME_BASE_Q,
                                 avctx->time_base);

      hi_logt(avctx, "%s %d AMediaCodec OutputBuffer status: %d outsize: %u flags: %d offset: %d size: %d pts: [%lld %lld] nalu: [%x %x %x %x %x %x]",
              __FUNCTION__, __LINE__, ou_bufidx, ou_bufsize, bufferInfo.flags, bufferInfo.offset, bufferInfo.size, bufferInfo.presentationTimeUs,
              pts, ou_buf[0], ou_buf[1], ou_buf[2], ou_buf[3], ou_buf[4], ou_buf[5]);

      if ((ret = ff_alloc_packet(avctx, pkt, bufferInfo.size) < 0))
      {
        hi_loge(avctx, "ff_alloc_packet fail(%d)", ret);
        AMediaCodec_releaseOutputBuffer(ctx->mediacodec, ou_bufidx, false);
        ret = AVERROR_EXTERNAL;
        break;
      }
    ...
    {
      hi_logd(avctx, "%s %d ou_bufidx %d AMediaCodec_dequeueOutputBuffer AMEDIACODEC_INFO_TRY_AGAIN_LATER ",
              __FUNCTION__, __LINE__, ou_bufidx);

      ctx->stats.ou_fail_again_cnt++;

      if (ou_times <= 0)
      {
        hi_logw(avctx, "%s %d ou_times %d ret %d AMediaCodec_dequeueOutputBuffer timeout ", __FUNCTION__, __LINE__, ou_times, ret);
        ret = AVERROR(EAGAIN);
      }
      // hi_logd(avctx, "%s %d ret %d againLaterCount %d ", __FUNCTION__, __LINE__, ret, againLaterCount++);
      continue;
    }
    else if (ou_bufidx == AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED)
    {
      ...
      if (format)
      {
        hi_logi(avctx, "%s %d AMediaCodec_dequeueOutputBuffer AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED %s", __FUNCTION__, __LINE__, AMediaFormat_toString(format));
  ...
  // hi_logd(avctx, "%s %d received packet (%d) %d", __FUNCTION__, __LINE__, ret, receivePacket++);
  return ret;
}

static int hlmediacodec_encode_receive_packet(AVCodecContext *avctx, AVPacket *pkt, const AVFrame *frame, int *got_packet)
{
  hi_logd(avctx, "%s %d frame address %p pkt address %p pkt->size %d got_packet '%d'.\n", __FUNCTION__, __LINE__, frame, pkt, pkt->size, *got_packet);
  HLMediaCodecEncContext *ctx = avctx->priv_data;
...
{
  hi_logi(avctx, " %s %d hlmediacodec_encode_close", __FUNCTION__, __LINE__);
...
static const AVClass ff_h264_hlmediacodec_enc_class = {
    .class_name = "h264_hlmediacodec",
    .item_name = av_default_item_name,
    .option = ff_hlmediacodec_enc_options,
    .version = LIBAVUTIL_VERSION_INT,
};

const FFCodec ff_h264_hlmediacodec_encoder = {
    .p.name = "h264_hlmediacodec",
    .p.long_name = NULL_IF_CONFIG_SMALL("Ffmpeg MediaCodec NDK"),
    .p.type = AVMEDIA_TYPE_VIDEO,
    .p.id = AV_CODEC_ID_H264,
    .p.priv_class = &ff_h264_hlmediacodec_enc_class,
    .priv_data_size = sizeof(HLMediaCodecEncContext),
    .init = hlmediacodec_encode_init,
    FF_CODEC_ENCODE_CB(hlmediacodec_encode_receive_packet),
    .close = hlmediacodec_encode_close,
    .p.capabilities = AV_CODEC_CAP_DELAY,
    .caps_internal = FF_CODEC_CAP_INIT_THREADSAFE | FF_CODEC_CAP_INIT_CLEANUP,
    .p.pix_fmts =
        (const enum AVPixelFormat[]){AV_PIX_FMT_NV12, AV_PIX_FMT_YUV420P,
                                     AV_PIX_FMT_NONE},
};
